<template>
  ...
  <el-form-item label="代码字段" prop="params.codeFieldId" :rules="req('代码字段')">
    <el-select v-model="model.codeFieldId" placeholder="选择代码字段" filterable clearable :loading="loadingElements" :disabled="!model.datasetId" style="width:100%">
      <el-option v-for="el in elements" :key="el.id" :label="elementLabel(el)" :value="el.id" />
    </el-select>
  </el-form-item>
  <el-form-item label="名称字段" prop="params.nameFieldId" :rules="req('名称字段')">
    <el-select v-model="model.nameFieldId" placeholder="选择名称字段" filterable clearable :loading="loadingElements" :disabled="!model.datasetId" style="width:100%">
      <el-option v-for="el in elements" :key="el.id" :label="elementLabel(el)" :value="el.id" />
    </el-select>
  </el-form-item>
...
const onDS = () => { model.value.codeFieldId = null; model.value.nameFieldId = null; fetchElements(model.value.datasetId) }

const fetchCodesets = async () => {
  loadingCodesets.value = true
  try { const { data } = await axios.get('/api/codeset', { params: { page: 1, size: 200 } }); codesets.value = data.data?.records || [] } finally { loadingCodesets.value = false }
}
fetchCodesets()
